...
void AMagicCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// Ability mappings
	PlayerInputComponent->BindAction("Ability1", IE_Pressed, this, &AMagicCharacter::Ability1_Heal);
	PlayerInputComponent->BindAction("Ability2", IE_Pressed, this, &AMagicCharacter::Ability2_Projectile);
...
void AMagicCharacter::Ability1_Heal()
{
	...
	if (Ability1Cooldown >= 1.f)
	{
		...
		if (bCasting1H == false && bCasting1HCombat == false)
		{
			if (Mana >= 0.15f)
			{
				...
				//ResetCastingStateDel = FTimerDelegate::CreateUObject(this, &AMagicCharacter::ResetCastingState, bCasting1H);
...
void AMagicCharacter::Ability2_Projectile()
{
	// If cooldown is 1, then we are allowed to cast
...
void AMagicCharacter::ResetCastingState(bool &bCastingState)
{
...
void AMagicCharacter::ResetAbilityCooldown(float &AbilityCooldown)
{
...
void AMagicCharacter::HealthRegen(float DeltaTime)
{
	if (Health < 1.f)
	{
...
void AMagicCharacter::ManaRegen(float DeltaTime)
{
	if (Mana < 1.f)
	{
...
void AMagicCharacter::NoManaPrompt()
{
	if(NoManaWidget)
	{ 
...
void AMagicCharacter::ModifyHealth(float Modifier)
{
	Health += Modifier;
	if (Health > 1.f)
	{
...
void AMagicCharacter::ModifyMana(float Modifier)
{
	Mana += Modifier;
	if (Mana > 1.f)
	{
...
bool AMagicCharacter::IsHealthFull()
{
	if (Health == 1.f)
	{
		return true;
